...
async function queryStabilityAIImage(data, apiKey) {
    ...
    const payload = {
        text_prompts: [
            ...
            { text: "blurry, low quality, colored, modern, digital, photograph, watermark, signature, text, letters", weight: -1.0 } 
        ],
        cfg_scale: 7,
        height: 768,   
        width: 1344,  
...
export default async function handler(request, response) {
  // Allow requests from any origin (useful for local development)
  ...
  const FIREBASE_CONFIG = process.env.FIREBASE_CONFIG; // Use the correct variable name
    ...
    switch (type) {
      ...
      case 'image':
        ...
        if (!aiImageResponse.ok) {
            ...
            return response.status(aiImageResponse.status).json({ error: `Stability AI Error: ${errorText}` });
        }
